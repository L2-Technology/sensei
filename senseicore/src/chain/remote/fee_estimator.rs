...
impl RemoteFeeEstimator {
  ...
  fn fee_rate_background_path(&self) -> String {
    format!("{}/v1/ldk/chain/fee-rate-background", self.remote_sensei.host)
  }

  fn fee_rate_high_priority_path(&self) -> String {
    format!("{}/v1/ldk/chain/fee-rate-high-priority", self.remote_sensei.host)
  }

  pub async fn get_fee_rate_normal(&self) -> u32 {
    let client = reqwest::Client::new();
    match client.get(self.fee_rate_normal_path())
      ...
      .await {
        Ok(response) => {
          match response.text().await {
            Ok(fee_rate_string) => {
              fee_rate_string.parse().unwrap_or(2000)
            },
            Err(_) => 2000
          }
        },
  ...
  pub async fn get_fee_rate_background(&self) -> u32 {
    let client = reqwest::Client::new();
    match client.get(self.fee_rate_background_path())
      .header("token", self.remote_sensei.token.clone())
      .send()
      .await {
        Ok(response) => {
          match response.text().await {
            Ok(fee_rate_string) => {
              fee_rate_string.parse().unwrap_or(253)
            },
            Err(_) => 253
          }
        },
  ...
  pub async fn get_fee_rate_high_priority(&self) -> u32 {
    let client = reqwest::Client::new();
    match client.get(self.fee_rate_high_priority_path())
      .header("token", self.remote_sensei.token.clone())
      .send()
      .await {
        Ok(response) => {
          match response.text().await {
            Ok(fee_rate_string) => {
              fee_rate_string.parse().unwrap_or(5000)
            },
            Err(_) => 5000
          }
        },
